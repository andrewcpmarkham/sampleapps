...
class LocationCollection {
    ...
    static func saveToFile(locations: [Location]) {
        ...
        let locationDTOs = locations.compactMap({LocationDTO(from: $0)})

        let propertyListEncoder = PropertyListEncoder()
        let encodedEmoji = try? propertyListEncoder.encode(locationDTOs)
    ...
    static func loadFromFile() -> [Location] {
        ...
        if
            ...
            let decodedDTOs = try? propertyListDecoder.decode(Array<LocationDTO>.self, from: retrievedLocationData) {
            return decodedDTOs.compactMap({Location.init(from:$0)})
        }
        return []
    }
}
