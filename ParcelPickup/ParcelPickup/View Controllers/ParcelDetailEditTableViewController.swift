//
//  ParcelDetailEditTableViewController.swift
//  ParcelPickup
//
//  Created by Andrew CP Markham on 2/8/20.
//  Copyright © 2020 Xercise Pro. All rights reserved.
//

import UIKit

class ParcelDetailEditTableViewController: UITableViewController {
      //Outlets
    ...
    //Functions
    func updateView(){
        ///Function to set all outlets/data points approprietly
        if let parcel = parcel{
            updateStatus(status: parcel.status, statusUpdateDate: parcel.statusUpdatedDate)
            statusDatePickerView.date = parcel.statusUpdatedDate
            datePickerChanged(statusDatePickerView)
            trackingNoTextField.text = parcel.trackingNo
            recipientNameTextField.text = parcel.recipientName
            deliveryAddressTextField.text = parcel.deliveryAddress
            if let deliveryTimeStamp = parcel.deliveryDate{
                deliveryDatePickerView.date = deliveryTimeStamp
                datePickerChanged(deliveryDatePickerView)
            }else{
                deliveryDateLabel.text = Status.isNotCompleted()
            }
            if let notes = parcel.notes{
    ...
    func updateStatus(status: Status, statusUpdateDate: Date){
        ...
        if Status.isCompleted(status: status){
            deliveryDatePickerView.date = statusUpdateDate
            datePickerChanged(deliveryDatePickerView)
        }else{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == PropertyKeys.segueEditStatus{
        ...
        }else if(segue.identifier == PropertyKeys.segueSaveEditUnwind){
        ...
        }else if segue.identifier == PropertyKeys.segueDeleteUnwind{
            deleteParcel = true
        }
    }
...
extension ParcelDetailEditTableViewController: SelectStatusTableViewControllerDelegate{
    //Protocol Implementation
        
    func didSelectStatus(status: Status){
        //Function for when a status is selected/changed
       updateStatus(status: status, statusUpdateDate: Date())
    }
}
