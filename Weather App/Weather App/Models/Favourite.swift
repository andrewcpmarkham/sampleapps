...
class Favourite: Codable{
    ...
    private static let archiveURL = documentsDirectory.appendingPathComponent("favourite").appendingPathExtension("plist")
    ...
    static func loadFromFile() -> Favourite?{
        ...
        if let retrievedLocationData = try? Data(contentsOf: archiveURL), let decodedFavourite = try? propertyListDecoder.decode(Favourite.self, from: retrievedLocationData){
