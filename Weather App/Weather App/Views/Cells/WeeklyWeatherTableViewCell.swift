...
class WeeklyWeatherTableViewCell: UITableViewCell {
    ...
    func updateWeekWeatherCell(with weather: WeatherResponse, index: Int){
        ...
        if let icon = weather.dailyWeather[index].weather.first?.icon , let iconURL = GetWeatherFromAPIDelegate.getIconURL(with: icon){
            //get weather icon
        ...
        dateLabel.text = Date.dateOnlyFormatter.string(from: Date.dateFromUTCInt(UTCTimeStamp: weather.dailyWeather[index].dt + weather.timezoneOffset))
