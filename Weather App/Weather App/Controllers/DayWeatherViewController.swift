...
class DayWeatherViewController: UIViewController {
    ...
    func updateUIWithWeatherFromAPI() {
        ...
        if let weather = location.weather, let todaysWeather = location.weather?.dailyWeather[1], let icon = todaysWeather.weather.first?.icon, let iconURL = GetWeatherFromAPIDelegate.getIconURL(with: icon){
            //Set known Data
            dateLabel.text = Date.dateOnlyFormatter.string(from: Date.dateFromUTCInt(UTCTimeStamp: todaysWeather.dt + weather.timezoneOffset))
